...
function _daysUntilExpiry(iso: string | null | undefined): number | null {
    if (!iso) return null;
    try {
...
export function filterSslCertificates(): void {
    const q = ((document.getElementById("ssl-search") as HTMLInputElement).value || "").toLowerCase();
    renderSslCertificates(
        state.allSslCertificates.filter((c) => {
...
export function renderSslCertificates(list: SslCertificate[]): void {
    const grid = document.getElementById("ssl-certificates-grid") as HTMLElement;
    const empty = document.getElementById("ssl-certificates-empty") as HTMLElement;
    if (!list.length) {
        grid.innerHTML = "";
    ...
    grid.innerHTML = list
        .map((c) => {
            const altDomains = c.alt_domains
                ? c.alt_domains
                    .split(",")
                    .map((d) => d.trim())
                    .filter((d) => d)
            ...
            details.push(
                `<div class="sc-detail-item"><span class="sc-detail-icon">${SC_IC.mail}</span><span class="sc-detail-label">Email</span><span class="sc-detail-value${!c.email ? " muted" : ""}">${c.email ? escHtml(c.email) : "not set"}</span></div>`,
            );
            details.push(
                `<div class="sc-detail-item"><span class="sc-detail-icon">${SC_IC.calendar}</span><span class="sc-detail-label">Issued</span><span class="sc-detail-value${!c.issued_at ? " muted" : ""}">${_formatDate(c.issued_at)}</span></div>`,
            );
            details.push(
                `<div class="sc-detail-item"><span class="sc-detail-icon">${SC_IC.calendar}</span><span class="sc-detail-label">Expires</span><span class="sc-detail-value" style="${c.status === "expired" || expiryWarn ? "color:var(--danger)" : ""}">${_formatDate(c.expires_at)}${daysLeft !== null ? ` (${daysLeft}d)` : ""}</span></div>`,
            );
            if (c.last_renewal_at)
                details.push(
                    `<div class="sc-detail-item"><span class="sc-detail-icon">${SC_IC.refresh}</span><span class="sc-detail-label">Renewed</span><span class="sc-detail-value">${_formatDate(c.last_renewal_at)}</span></div>`,
                );
...
export async function openSslCertificateModal(existing: Partial<SslCertificate> | null = null): Promise<void> {
    const c = existing || {};
    const isNew = !c.id;

    let aclDomains: string[] = [];
    try {
        aclDomains = await api("/api/ssl-certificates/acl-domains");
    } catch {
        /* ignore */
    }

    const altDomainsArr = c.alt_domains
        ? c.alt_domains
            .split(",")
            .map((d) => d.trim())
            .filter((d) => d)
    ...
    openModal(
        ...
        <div class="sc-domain-picker">
            ${aclDomains.length
            ? `<div class="sc-domain-picker-tabs" id="sc-domain-tabs">
            ...
            ${aclDomains.length
            ? `<div id="sc-domain-acl" style="display:${isNew ? "block" : "none"}">
...
export function switchDomainTab(tab: string): void {
    document.querySelectorAll(".sc-domain-picker-tab").forEach((t) => t.classList.remove("active"));
    const customDiv = document.getElementById("sc-domain-custom") as HTMLElement;
    const aclDiv = document.getElementById("sc-domain-acl") as HTMLElement | null;
    if (tab === "acl" && aclDiv) {
        document.querySelectorAll(".sc-domain-picker-tab")[1]?.classList.add("active");
...
export function selectAclDomain(el: HTMLElement, domain: string): void {
    document.querySelectorAll(".sc-acl-domain-item").forEach((i) => i.classList.remove("selected"));
    el.classList.add("selected");
    (document.getElementById("m-domain") as HTMLInputElement).value = domain;
    switchDomainTab("custom");
}

/** Filters ACL domain items in the picker grid by search query. */
export function filterAclDomains(q: string): void {
    q = q.toLowerCase();
    document.querySelectorAll<HTMLElement>(".sc-acl-domain-item").forEach((el) => {
        el.style.display = (el.textContent || "").toLowerCase().includes(q) ? "" : "none";
    });
}

/** Adds an alternative domain (SAN) tag from the input field. */
export function addAltDomain(): void {
    const inp = document.getElementById("m-alt-domain-input") as HTMLInputElement;
...
export function removeAltDomain(el: HTMLElement): void {
    el.parentElement?.remove();
}

/** Toggles a mini ACL domain picker for selecting alternative domains. */
export function showAltDomainAclPicker(): void {
...
export function pickAltAclDomain(el: HTMLElement, domain: string): void {
    const list = document.getElementById("sc-alt-domains-list") as HTMLElement;
    const existing = [...list.querySelectorAll(".sc-alt-domain-tag")].map((t) => (t.textContent || "").replace("\u00d7", "").trim());
...
export function onProviderChange(): void {
    const prov = (document.getElementById("m-provider") as HTMLSelectElement).value;
...
export function autofillCertPaths(): void {
    const domain = ((document.getElementById("m-domain") as HTMLInputElement).value || "").trim();
    if (!domain) {
    ...
    (document.getElementById("m-cert-path") as HTMLInputElement).value = `${base}/cert.pem`;
    (document.getElementById("m-key-path") as HTMLInputElement).value = `${base}/privkey.pem`;
    (document.getElementById("m-fullchain-path") as HTMLInputElement).value = `${base}/fullchain.pem`;
    toast("Paths auto-filled");
}

/** Toggles DNS plugin input visibility based on challenge type selection. */
export function toggleDnsPlugin(): void {
    const ct = (document.getElementById("m-challenge-type") as HTMLSelectElement).value;
    (document.getElementById("dns-plugin-wrap") as HTMLElement).style.display = ct === "dns-01" ? "block" : "none";
}

/** Saves a new or updated SSL certificate with all fields from the 3-step form. */
export async function saveSslCertificate(id: number | null): Promise<void> {
    const altTags = document.querySelectorAll("#sc-alt-domains-list .sc-alt-domain-tag");
    const altDomains = [...altTags]
    ...
    const body = {
        ...
        email: (document.getElementById(isCertbot ? "m-email" : "m-email-manual") as HTMLInputElement | null)?.value || null,
        provider: prov,
        status: (document.getElementById("m-status") as HTMLSelectElement).value,
        challenge_type: isCertbot
            ? (document.getElementById("m-challenge-type") as HTMLSelectElement | null)?.value || "http-01"
            : (document.getElementById("m-challenge-type-manual") as HTMLSelectElement | null)?.value || "http-01",
        dns_plugin: (document.getElementById("m-dns-plugin") as HTMLInputElement | null)?.value || null,
        cert_path: (document.getElementById("m-cert-path") as HTMLInputElement | null)?.value || null,
        key_path: (document.getElementById("m-key-path") as HTMLInputElement | null)?.value || null,
        fullchain_path: (document.getElementById("m-fullchain-path") as HTMLInputElement | null)?.value || null,
        issued_at: (document.getElementById("m-issued-at") as HTMLInputElement | null)?.value || null,
        expires_at: (document.getElementById("m-expires-at") as HTMLInputElement | null)?.value || null,
        auto_renew: (document.getElementById("m-auto-renew") as HTMLInputElement).checked,
