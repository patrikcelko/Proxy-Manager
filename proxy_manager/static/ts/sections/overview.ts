    ...
    try {
      ...
      <div class="fn-detail">HTTP / HTTPS / TCP</div>
        ...
        const aclCol = sortedAcls.length
            ? sortedAcls
                .map(
                    (a: any) => `<div class="flow-node acl" data-backend="${escHtml(a.backend_name || "")}" data-fid="${a.frontend_id || ""}">
        ...
        const authCol = userlists.length
            ? userlists
                .map(
                    (u: any) => `<div class="flow-node auth-ul" data-name="${escHtml(u.name)}" style="border-left-color:#f472b6">
        ...
        const srvCol = beOrder.length
            ? beOrder
                .flatMap((b: any) =>
                    (b.servers || []).map(
                        (s: any) => `<div class="flow-node srv" data-backend="${escHtml(b.name)}">
        ...
        const sepDown = `<div class="flow-separator-v">${icon("arrow-down-flow", 24, 1.5)}</div>`;
        ...
        <div class="flow-row">
          ...
          <div class="flow-row-nodes">${clientCol}
            ${listens.length
                ? listens
                    .map(
                        (l: any) => `<div class="flow-node listen-n">
...
function bezierV(from: FlowPoint, to: FlowPoint, color: string, opacity: number = 0.25, width: number = 1.5): string {
...
export function drawFlowConnections(): void {
    ...
    if (clientNode) {
        diagram.querySelectorAll(".flow-node.fe").forEach((feNode) => {
            paths += bezierV(nodeEdge(clientNode, "bottom", dr), nodeEdge(feNode, "top", dr), "#60a5fa", 0.2);
        });
    }
    ...
    diagram.querySelectorAll(".flow-node.acl").forEach((aclNode) => {
        ...
        if (fid) {
            ...
            if (feNode) {
                paths += bezierV(nodeEdge(feNode, "bottom", dr), nodeEdge(aclNode, "top", dr), "#4ade80", 0.25);
            }
        }
    });
    ...
    diagram.querySelectorAll(".flow-node.fe").forEach((feNode) => {
        ...
        if (defBe) {
            ...
            if (beNode) {
                paths += bezierV(nodeEdge(feNode, "bottom", dr), nodeEdge(beNode, "top", dr), "#4ade80", 0.15, 1);
            }
        }
    });
    ...
    diagram.querySelectorAll(".flow-node.acl").forEach((aclNode) => {
        ...
        if (bName) {
            ...
            if (beNode) {
                paths += bezierV(nodeEdge(aclNode, "bottom", dr), nodeEdge(beNode, "top", dr), "#f87171", 0.25);
            }
        }
    });
    ...
    diagram.querySelectorAll(".flow-node.be").forEach((beNode) => {
        ...
        if (authName) {
            ...
            if (authNode) {
                paths += bezierV(nodeEdge(authNode, "bottom", dr), nodeEdge(beNode, "top", dr), "#f472b6", 0.3, 1.5);
            }
        }
    });
    ...
    diagram.querySelectorAll(".flow-node.srv").forEach((srvNode) => {
        ...
        if (beName) {
            ...
            if (beNode) {
                paths += bezierV(nodeEdge(beNode, "bottom", dr), nodeEdge(srvNode, "top", dr), "#fbbf24", 0.2);
            }
        }
    });
