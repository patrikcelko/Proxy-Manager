...
/* Global list of all mailer sections, populated by loadMailers() */
let allMailers = [];

/* Renders mailer section cards with entry sub-cards showing AUTH/TLS/STARTTLS badges */
function renderMailersGrid(items) {
    const grid = document.getElementById('mailers-grid');
    const empty = document.getElementById('mailers-empty');
...
async function loadMailers() {
    try {
        ...
        allMailers = d.items || d;
        renderMailersGrid(allMailers);
    } catch (err) { toast(err.message, 'error'); }
}

/* Filters mailer sections by name, comment, or entry name/address */
function filterMailers() {
    const q = (document.getElementById('mailer-search').value || '').toLowerCase();
    if (!q) { renderMailersGrid(allMailers); return; }
    renderMailersGrid(allMailers.filter(m =>
...
function openMailerModal(existing = null) {
    const m = existing || {};
    const SEC = {
        mail: icon('mail', 15),
...
function openMailerEntryModal(mailerId, existing = null) {
    const e = existing || {};
    const SEC = {
...
async function saveMailer(id) {
    const body = {
        name: document.getElementById('m-name').value,
        timeout_mail: document.getElementById('m-timeout-mail').value || null,
    ...
    try {
        ...
        closeModal(); toast(id ? 'Updated' : 'Created'); loadMailers();
    } catch (err) { toast(err.message, 'error'); }
}
...
async function saveMailerEntry(mailerId, entryId) {
    const smtpAuth = document.getElementById('m-smtp-auth').checked;
    const body = {
        name: document.getElementById('m-name').value,
        address: document.getElementById('m-address').value,
        port: parseInt(document.getElementById('m-port').value) || 25,
        sort_order: parseInt(document.getElementById('m-sort').value) || 0,
        smtp_auth: smtpAuth,
        smtp_user: smtpAuth ? (document.getElementById('m-smtp-user').value || null) : null,
        smtp_password: smtpAuth ? (document.getElementById('m-smtp-password').value || null) : null,
        use_tls: document.getElementById('m-use-tls').checked,
        use_starttls: document.getElementById('m-use-starttls').checked,
    };
    try {
        ...
        closeModal(); toast('Saved'); loadMailers();
    } catch (err) { toast(err.message, 'error'); }
}

/* Deletes a mailer entry after confirmation */
async function deleteMailerEntry(mailerId, entryId) {
