...
/* Global list of all ACL rules, populated by loadAclRules() */
let allAclRules = [];
...
async function loadAclRules() {
    try {
        ...
        allAclRules = d.items || d;
        renderAclTable(allAclRules);
    } catch (err) { toast(err.message, 'error'); }
}

/* Filters ACL rules by search query across domain, backend, redirect, match type, comment, and frontend name */
function filterAclTable() {
    aclFilter = (document.getElementById('acl-search').value || '').toLowerCase();
    renderAclTable(allAclRules.filter(a =>
...
function renderAclTable(list) {
    ...
    tbody.innerHTML = list.map((a, idx) => {
        const feLabel = allFrontends.find(f => f.id === a.frontend_id)?.name || '-';
        const isFirst = idx === 0;
        ...
        const matchIcons = {
            'hdr': 'header', 'hdr_beg': 'header', 'hdr_end': 'header', 'hdr_reg': 'header', 'hdr_dom': 'domain',
            'path': 'path', 'path_beg': 'path', 'path_end': 'path', 'path_reg': 'path',
            'src': 'ip', 'url_beg': 'url', 'url_reg': 'url'
        };
        const matchType = a.acl_match_type || 'hdr';
        const matchCat = matchIcons[matchType] || 'header';
        const matchBadgeClass = matchCat === 'domain' ? 'badge-info' : matchCat === 'path' ? 'badge-warn' : matchCat === 'ip' ? 'badge-muted' : 'badge-muted';
...
function openAclModal(existing = null) {
    const a = existing || {};
    const feOpts = allFrontends.map(f => `<option value="${f.id}" ${a.frontend_id === f.id ? 'selected' : ''}>${escHtml(f.name)}</option>`).join('');
    const beOpts = allBackends.map(b => `<option value="${escHtml(b.name)}" ${a.backend_name === b.name ? 'selected' : ''}>${escHtml(b.name)}</option>`).join('');
    ...
    openModal(`
        ...
        <div><label>Match Type</label><select id="m-match">
            ${['hdr', 'hdr_beg', 'hdr_end', 'hdr_reg', 'hdr_dom', 'path', 'path_beg', 'path_end', 'path_reg', 'src', 'url_beg', 'url_reg'].map(m =>
        `<option ${a.acl_match_type === m ? 'selected' : ''}>${m}</option>`).join('')}
            </select>
...
function toggleAclRedirect(isRedir) {
    document.getElementById('acl-backend-row').style.display = isRedir ? 'none' : '';
    document.getElementById('acl-redirect-row').style.display = isRedir ? '' : 'none';
}

/* Saves a new or updated ACL rule with domain, match type, action, and scope */
async function saveAclRule(id) {
    const isRedir = document.getElementById('m-is-redirect').value === 'true';
    const body = {
        domain: document.getElementById('m-domain').value,
        acl_match_type: document.getElementById('m-match').value,
        frontend_id: parseInt(document.getElementById('m-frontend').value) || null,
        is_redirect: isRedir,
        backend_name: isRedir ? null : document.getElementById('m-backend').value,
        redirect_target: isRedir ? document.getElementById('m-redirect').value : null,
        redirect_code: isRedir ? safeInt(document.getElementById('m-redir-code').value) : null,
        enabled: document.getElementById('m-enabled').value === 'true',
    ...
    try {
        ...
        closeModal(); toast(id ? 'Updated' : 'Created'); loadAclRules();
    } catch (err) { toast(err.message, 'error'); }
}
...
async function reorderAclRule(id, direction) {
    const sorted = [...allAclRules].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
