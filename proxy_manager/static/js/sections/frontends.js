...
options/directives, categorized option tabs, IP access rules,
bind preset templates, and option preset templates.
*/

/* Global list of all frontends, populated by loadFrontends() */
let allFrontends = [];
...
async function loadFrontends() {
    try {
        ...
        allFrontends = d.items || d;
        renderFrontends(allFrontends);
...
function filterFrontends() {
    const q = (document.getElementById('frontend-search').value || '').toLowerCase();
    if (!q) { renderFrontends(allFrontends); return; }
    renderFrontends(allFrontends.filter(f =>
...
function categorizeFrontendOpt(dir) {
...
/* Renders bind addresses as colored chips, splitting multi-bind lines and extracting shared SSL suffix */
function renderBindChips(bindLine) {
    ...
    const addrs = sharedSuffix
        ? addrParts.map(a => a.addr)
        : parts;
    return addrs.map(a => `<span class="bind-chip">${escHtml(a)}</span>`).join('')
        + (sharedSuffix ? `<span class="bind-opts-suffix">${escHtml(sharedSuffix)}</span>` : '');
}

/* Renders the complete frontends card grid with entity cards, bind lists, option tabs, and IP access rules */
function renderFrontends(list) {
    const grid = document.getElementById('frontends-grid');
    const empty = document.getElementById('frontends-empty');
    ...
    grid.innerHTML = list.map(f => {
        const bc = (f.binds || []).length, oc = (f.options || []).length;

        const hasSSL = (f.binds || []).some(b => /\bssl\b/i.test(b.bind_line));
        const ports = [...new Set((f.binds || []).flatMap(b => {
        ...
        const catTabs = Object.entries(FE_OPT_CATS).map(([k, v]) => {
        ...
        return `<div class="entity-card">
            ...
            <div class="entity-body">
                ${details.length ? `<div class="entity-section fe-details-section">
                    <div class="fe-detail-grid">${details.map(d => `<div class="fe-detail-item"><span class="fe-detail-label">${d.l}</span><span class="fe-detail-value">${escHtml(d.v)}</span></div>`).join('')}</div>
                </div>` : ''}
                ...
                <div class="entity-section">
                    ...
                    ${regularOpts.length > 3 ? `<div class="fe-opt-search preset-search-wrap" style="margin-bottom:.5rem">
                        ${icon('search')}
                        <input placeholder="Search options..." oninput="searchFeOpts(${f.id}, this.value)">
                    </div>` : ''}
                ...
                ${ipRules.length ? `<div class="entity-section">
                    <div class="entity-section-head">
                        <span class="entity-section-label">${FI.shield} IP Access Rules (${ipRules.length})</span>
                    </div>
                    <ul class="entity-list" id="${feIpId}">${ipList}</ul>
                </div>` : ''}
...
function filterFeOpts(fid, cat) {
    const list = document.getElementById(`fe-opts-${fid}`);
    if (!list) return;
    const section = list.closest('.entity-section');
    const searchInput = section?.querySelector('.fe-opt-search input');
    if (searchInput) searchInput.value = '';
    const tabsWrap = section?.querySelector('.fe-opt-tabs');
    if (tabsWrap) tabsWrap.querySelectorAll('.stab').forEach(t => t.classList.toggle('active', t.textContent.trim().startsWith(cat === 'all' ? 'All' : (FE_OPT_CATS[cat]?.label || ''))));
    list.querySelectorAll('li[data-fe-cat]').forEach(li => {
        li.style.display = (cat === 'all' || li.dataset.feCat === cat) ? '' : 'none';
    });
}

/* Filters options in a frontend card by free-text search query */
function searchFeOpts(fid, query) {
    const list = document.getElementById(`fe-opts-${fid}`);
    if (!list) return;
    ...
    if (!q) { filterFeOpts(fid, 'all'); return; }
    list.querySelectorAll('li[data-fe-cat]').forEach(li => {
...
function openFrontendModal(existing = null) {
    const f = existing || {};
    const beOpts = allBackends.map(b => `<option value="${escHtml(b.name)}" ${f.default_backend === b.name ? 'selected' : ''}>${escHtml(b.name)}</option>`).join('');
...
async function saveFrontend(id) {
    const body = {
        name: document.getElementById('m-name').value,
        mode: document.getElementById('m-mode').value,
        default_backend: document.getElementById('m-default-backend').value || null,
        ...
        timeout_http_request: document.getElementById('m-timeout-http-request').value || null,
        timeout_http_keep_alive: document.getElementById('m-timeout-http-keep-alive').value || null,
    ...
    try {
        ...
        closeModal(); toast(id ? 'Updated' : 'Created'); loadFrontends();
    } catch (err) { toast(err.message, 'error'); }
}

/* Deletes a frontend after user confirmation */
async function deleteFrontend(id) {
    await crudDelete(`/api/frontends/${id}`, 'Delete this frontend?', loadFrontends);
}

/* Preset bind line templates organized by protocol category */
const BIND_PRESETS = [
...
function openBindModal(frontendId, existing = null) {
    const b = existing || {};
...
async function saveBind(frontendId, bindId) {
    ...
    try {
        ...
        closeModal(); toast('Saved'); loadFrontends();
    } catch (err) { toast(err.message, 'error'); }
}

/* Deletes a bind from a frontend after confirmation */
async function deleteBind(frontendId, bindId) {
    await crudDelete(`/api/frontends/${frontendId}/binds/${bindId}`, 'Delete this bind?', loadFrontends);
}

/* Preset frontend option/directive templates organized by functional category */
...
function openOptionModal(frontendId, existing = null) {
    const o = existing || {};
...
async function deleteOption(frontendId, optionId) {
