...
/* Global list of all backends, populated by loadBackends() */
let allBackends = [];
...
async function loadBackends() {
    try {
        ...
        allBackends = d.items || d;
        renderBackends(allBackends);
...
function filterBackends() {
    backendFilter = (document.getElementById('backend-search').value || '').toLowerCase();
    renderBackends(allBackends.filter(b => {
...
function renderBackends(list) {
    ...
    const empty = document.getElementById('backends-empty');
    ...
    grid.innerHTML = list.map(b => {
        ...
        const detailHtml = detailRows.length ? `<div class="be-detail-section">
            <div class="be-detail-grid">${detailRows.map(([icon, l, v]) =>
            `<div class="be-detail-item"><span class="be-detail-icon">${IC[icon] || ''}</span><span class="be-detail-label">${escHtml(l)}</span><span class="be-detail-value">${escHtml(v)}</span></div>`
        ).join('')}</div>
        </div>` : '';
...
function openBackendModal(existing = null) {
    const b = existing || {};
    const ulOpts = (window._cachedUserlists || []).map(u =>
        `<option value="${escHtml(u.name)}" ${b.auth_userlist === u.name ? 'selected' : ''}>${escHtml(u.name)}</option>`
    ).join('');
    if (!window._cachedUserlists) {
        api('/api/userlists').then(d => {
            window._cachedUserlists = d.items || d;
            const sel = document.getElementById('m-auth-userlist');
            if (sel) {
                const current = sel.value;
                const opts = window._cachedUserlists.map(u =>
                    `<option value="${escHtml(u.name)}" ${current === u.name || b.auth_userlist === u.name ? 'selected' : ''}>${escHtml(u.name)}</option>`
                ).join('');
...
async function saveBackend(id) {
    const body = {
        name: document.getElementById('m-name').value,
        mode: document.getElementById('m-mode').value,
        balance: document.getElementById('m-balance').value,
        option_forwardfor: document.getElementById('m-forwardfor').checked,
        option_redispatch: document.getElementById('m-redispatch').checked,
        option_httplog: document.getElementById('m-httplog').checked,
        option_tcplog: document.getElementById('m-tcplog').checked,
        retries: parseInt(document.getElementById('m-retries').value) || null,
        retry_on: document.getElementById('m-retry-on').value || null,
        auth_userlist: document.getElementById('m-auth-userlist').value || null,
        health_check_enabled: document.getElementById('m-hc-enabled').checked,
        health_check_method: document.getElementById('m-hc-method').value || null,
        health_check_uri: document.getElementById('m-hc-uri').value || null,
        http_check_expect: document.getElementById('m-hc-expect').value || null,
        errorfile: document.getElementById('m-errorfile').value || null,
        ...
        timeout_queue: document.getElementById('m-timeout-queue').value || null,
        cookie: document.getElementById('m-cookie').value || null,
        http_reuse: document.getElementById('m-http-reuse').value || null,
        hash_type: document.getElementById('m-hash-type').value || null,
        compression_algo: document.getElementById('m-comp-algo').value || null,
        compression_type: document.getElementById('m-comp-type').value || null,
        default_server_options: document.getElementById('m-default-server').value || null,
    };
    try {
        ...
        closeModal(); toast(id ? 'Updated' : 'Created'); loadBackends();
    } catch (err) { toast(err.message, 'error'); }
}
...
function openServerModal(backendId, existing = null) {
    const s = existing || {};
    const SI = {
...
async function saveServer(backendId, serverId) {
    const body = {
        name: document.getElementById('m-name').value,
        address: document.getElementById('m-address').value,
        port: safeInt(document.getElementById('m-port').value),
        check_enabled: document.getElementById('m-check').checked,
        maxconn: parseInt(document.getElementById('m-maxconn').value) || null,
        maxqueue: parseInt(document.getElementById('m-maxqueue').value) || null,
        extra_params: document.getElementById('m-params').value || null,
        sort_order: parseInt(document.getElementById('m-sort').value) || 0,
        weight: parseInt(document.getElementById('m-weight').value) || null,
        ssl_enabled: document.getElementById('m-ssl').checked,
        ssl_verify: document.getElementById('m-ssl-verify').value || null,
        backup: document.getElementById('m-backup').checked,
        inter: document.getElementById('m-inter').value || null,
        fastinter: document.getElementById('m-fastinter').value || null,
        downinter: document.getElementById('m-downinter').value || null,
        rise: parseInt(document.getElementById('m-rise').value) || null,
        fall: parseInt(document.getElementById('m-fall').value) || null,
        cookie_value: document.getElementById('m-cookie-value').value || null,
        send_proxy: document.getElementById('m-send-proxy').checked,
        send_proxy_v2: document.getElementById('m-send-proxy-v2').checked,
        slowstart: document.getElementById('m-slowstart').value || null,
        resolve_prefer: document.getElementById('m-resolve-prefer').value || null,
        resolvers_ref: document.getElementById('m-resolvers').value || null,
        on_marked_down: document.getElementById('m-on-marked-down').value || null,
        disabled: document.getElementById('m-disabled').checked,
    };
    try {
        ...
        closeModal(); toast('Saved'); loadBackends();
    } catch (err) { toast(err.message, 'error'); }
}

/* Deletes a server from a backend after confirmation */
async function deleteServer(backendId, serverId) {
