...
/* Global list of all peer sections, populated by loadPeers() */
let allPeers = [];

/* Renders peer section cards with peer entries and feature badges */
function renderPeersGrid(items) {
...
function filterPeers() {
    ...
    if (!q) { renderPeersGrid(allPeers); return; }
    renderPeersGrid(allPeers.filter(p =>
...
function openPeerModal(existing = null) {
    const p = existing || {};
    const SEC = {
...
function openPeerEntryModal(peerId, existing = null) {
    const e = existing || {};
    const SEC = {
...
async function savePeer(id) {
    const body = {
        ...
        default_bind: document.getElementById('m-default-bind').value || null,
    ...
    try {
        ...
        closeModal(); toast(id ? 'Updated' : 'Created'); loadPeers();
    } catch (err) { toast(err.message, 'error'); }
}
...
async function savePeerEntry(peerId, entryId) {
    const body = {
        name: document.getElementById('m-name').value,
        address: document.getElementById('m-address').value,
        port: parseInt(document.getElementById('m-port').value) || 10000,
    ...
    try {
        ...
        closeModal(); toast('Saved'); loadPeers();
    } catch (err) { toast(err.message, 'error'); }
}

/* Deletes a peer entry after confirmation */
async function deletePeerEntry(peerId, entryId) {
